...
public class TestGameBoard {
    ...
    // Standard method used for all test cases except constructor tests
    private IGameBoard MakeGameBoard() {
        IGameBoard asTest = new GameBoard(5,5,3);
        return asTest;
    }

    // Factory method so code is portable across TestGameBoard and TestGameBoardMem
    // Method only used to test the constructor with different sizes
    private IGameBoard MakeGameBoard_changed_size(int r, int c, int to_win) {
        IGameBoard asTest = new GameBoard(r,c,to_win);
        return asTest;
    }
